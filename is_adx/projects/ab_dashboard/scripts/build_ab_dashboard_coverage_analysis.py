...
def build_page_payload() -> dict:
    return shared.build_coverage_analysis_payload()
...
def write_page(write_html=shared.write_validated_html) -> Path:
    ...
    write_html(
        shared.COVERAGE_ANALYSIS_HTML,
        build_page_html(payload),
        required_strings=["覆盖率分析", shared.COVERAGE_HERO_TEXT[0], shared.COVERAGE_TEXT["metric1"][0]],
    )
    return shared.COVERAGE_ANALYSIS_HTML
