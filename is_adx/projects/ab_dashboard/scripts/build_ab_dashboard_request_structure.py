...
def build_page_payload() -> dict:
    return shared.build_request_structure_payload()
...
def build_page_html(payload: dict) -> str:
    return inject_home_button(
        shared.build_dashboard_html(
            ...
            success_scopes=payload.get("success_scopes", []),
...
def write_page(write_html=shared.write_validated_html) -> Path:
    ...
    write_html(
        shared.REQUEST_STRUCTURE_HTML,
        build_page_html(payload),
        required_strings=["请求结构分布", shared.REQUEST_STRUCTURE_HERO_TEXT[0], shared.REQUEST_STRUCTURE_TEXT["metric1"][0]],
    )
    return shared.REQUEST_STRUCTURE_HTML
