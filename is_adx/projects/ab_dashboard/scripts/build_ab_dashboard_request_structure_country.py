...
def build_page_payload() -> dict:
    return shared.build_request_structure_country_payload()
...
def build_page_html(payload: dict) -> str:
    return inject_home_button(
        shared.build_dashboard_html(
            ...
            country_options_by_combo=payload["country_options_by_combo"],
...
def write_page(write_html=shared.write_validated_html) -> Path:
    ...
    write_html(
        shared.REQUEST_STRUCTURE_COUNTRY_HTML,
        build_page_html(payload),
        required_strings=["请求结构分布（Country）", shared.REQUEST_STRUCTURE_COUNTRY_HERO_TEXT[0], shared.REQUEST_STRUCTURE_TEXT["metric1"][0]],
    )
    return shared.REQUEST_STRUCTURE_COUNTRY_HTML
