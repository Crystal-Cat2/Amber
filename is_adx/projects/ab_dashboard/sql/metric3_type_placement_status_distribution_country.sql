-- 查询目的：固定 total placement_cnt、network_type 与 type_placement_cnt 后，统计 status 的分布，并新增 country 维度。
-- 输出字段：experiment_group, product, ad_format, country, placement_cnt, network_type, type_placement_cnt, status_bucket, request_pv, denominator_request_pv, share
...
type_request_counts AS (
  ...
  GROUP BY p.experiment_group, p.product, p.ad_format, p.country, p.request_id, p.placement_cnt, d.network_type
),
...
denominator_totals AS (
  ...
  GROUP BY experiment_group, product, ad_format, country, placement_cnt, network_type, type_placement_cnt
),
...
status_counts AS (
  ...
  GROUP BY experiment_group, product, ad_format, country, placement_cnt, network_type, type_placement_cnt, status_bucket
)
