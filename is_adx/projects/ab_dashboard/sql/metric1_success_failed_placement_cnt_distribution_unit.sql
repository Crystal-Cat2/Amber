-- 查询目的：统计成功+失败 placement 的 request 级 placement_cnt 分布，并新增 max_unit_id 维度。
-- 输出字段：experiment_group, product, ad_format, max_unit_id, placement_cnt_bucket, request_pv, denominator_request_pv, total_request_pv, share
...
SELECT
  ...
  COUNT(*) AS request_pv,
  SUM(COUNT(*)) OVER (
    PARTITION BY p.experiment_group, p.product, p.ad_format, p.max_unit_id
...
GROUP BY p.experiment_group, p.product, p.ad_format, p.max_unit_id, placement_cnt_bucket
ORDER BY
  p.experiment_group,
  p.product,
  p.ad_format,
