-- 查询目的：按 country 固定 network_cnt 与 Bx+Wy 结构后，按 status 下钻各 B/W-network 的 request 命中占比。
...
typed_request_network AS (
  SELECT DISTINCT experiment_group, product, ad_format, country, user_pseudo_id, request_id, network_type, network
  FROM base_latency
),
typed_request_status_network AS (
  SELECT DISTINCT experiment_group, product, ad_format, country, user_pseudo_id, request_id, status_bucket, network_type, network
  FROM base_latency
),
...
denominator_totals AS (
  SELECT experiment_group, product, ad_format, country, network_cnt, bidding_cnt, waterfall_cnt, COUNT(*) AS denominator_request_pv
...
target_counts AS (
  ...
  GROUP BY b.experiment_group, b.product, b.ad_format, b.country, b.network_cnt, b.bidding_cnt, b.waterfall_cnt, t.status_bucket, t.network_type, t.network
)
SELECT
  c.experiment_group,
  c.product,
...
ORDER BY c.product, c.ad_format, c.country, c.network_cnt, c.bidding_cnt, c.waterfall_cnt, c.status_bucket, c.experiment_group, c.request_pv DESC, c.network_type, c.network;
