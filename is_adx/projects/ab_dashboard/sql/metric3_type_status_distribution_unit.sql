-- 查询目的：固定 total network_cnt、network_type 与 type_network_cnt 后，统计三种 status 的分布，并新增 max_unit_id 维度。
-- 输出字段：experiment_group, product, ad_format, max_unit_id, network_cnt, network_type, type_network_cnt, status_bucket, request_pv, denominator_request_pv, share
...
type_request_counts AS (
  ...
  INNER JOIN typed_request_network r
    ...
    AND p.max_unit_id = r.max_unit_id
    AND p.user_pseudo_id = r.user_pseudo_id
    AND p.request_id = r.request_id
  GROUP BY p.experiment_group, p.product, p.ad_format, p.max_unit_id, p.user_pseudo_id, p.request_id, p.network_cnt, r.network_type
),
...
denominator_totals AS (
  ...
  GROUP BY experiment_group, product, ad_format, max_unit_id, network_cnt, network_type, type_network_cnt
),
...
status_counts AS (
  ...
  GROUP BY experiment_group, product, ad_format, max_unit_id, network_cnt, network_type, type_network_cnt, status_bucket
)
