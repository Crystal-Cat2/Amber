-- 查询目的：按 country 固定 placement_cnt 与 Bx+Wy 结构后，按 status 下钻各 placement_id 的 request 命中占比。
...
request_status_placement AS (
  SELECT DISTINCT experiment_group, product, ad_format, country, user_pseudo_id, request_id, status_bucket, placement_id
  FROM base_latency
),
...
denominator_totals AS (
  SELECT experiment_group, product, ad_format, country, placement_cnt, bidding_placement_cnt, waterfall_placement_cnt, COUNT(*) AS denominator_request_pv
...
target_counts AS (
  ...
  GROUP BY b.experiment_group, b.product, b.ad_format, b.country, b.placement_cnt, b.bidding_placement_cnt, b.waterfall_placement_cnt, t.status_bucket, t.placement_id
)
SELECT
  c.experiment_group,
  c.product,
...
ORDER BY c.product, c.ad_format, c.country, c.placement_cnt, c.bidding_placement_cnt, c.waterfall_placement_cnt, c.status_bucket, c.experiment_group, c.request_pv DESC, c.placement_id;
