-- 查询目的：统计成功 request 在不同 network_cnt / placement_cnt 桶内的 success_rank 分布，并新增 country 维度。
-- 输出字段：experiment_group, product, ad_format, country, cnt_type, cnt_value, success_rank, request_pv, bucket_success_request_pv, bucket_total_request_pv
...
dedup_rows AS (
  ...
  FROM (
    SELECT
      *,
      ROW_NUMBER() OVER (
        PARTITION BY experiment_group, product, ad_format, country, user_pseudo_id, request_id, status, network_type, network, placement_id
...
success_rank_rows AS (
  ...
  QUALIFY ROW_NUMBER() OVER (
    PARTITION BY experiment_group, product, ad_format, country, user_pseudo_id, request_id
    ORDER BY event_timestamp ASC
  ) = 1
),
